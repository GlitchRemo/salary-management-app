...
describe("EmployeeDetailsPage", () => {
  ...
  it("renders the employee name as heading", async () => {
    vi.mocked(findEmployeeById).mockResolvedValue(buildEmployee({ name: "Alice Smith" }));
  ...
  it("renders the employee email", async () => {
    vi.mocked(findEmployeeById).mockResolvedValue(buildEmployee({ email: "alice@acme.com" }));
    render(await EmployeeDetailsPage(buildParams("emp_1")));
  ...
  it("renders the employee department and country", async () => {
    vi.mocked(findEmployeeById).mockResolvedValue(
      buildEmployee({ department: "Engineering", country: "US" }),
    );
    render(await EmployeeDetailsPage(buildParams("emp_1")));
  ...
  it("renders base salary and bonus with currency", async () => {
    vi.mocked(findEmployeeById).mockResolvedValue(
      buildEmployee({ currency: "USD", baseSalary: 80000, bonus: 5000 }),
    );
    render(await EmployeeDetailsPage(buildParams("emp_1")));
